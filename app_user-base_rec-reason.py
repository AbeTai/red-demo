...
from models.matrix_factorization.user_based_recommender import UserBasedRecommender
...
# ページ設定
st.set_page_config(
    page_title="Music Recommender Demo (User-Based with Recommendation Reasons)",
    page_icon="🎵",
    layout="wide"
)

@st.cache_resource
def load_user_based_recommender(csv_path, alpha):
    """ユーザーベースレコメンダーを読み込み（キャッシュ）"""
    recommender = UserBasedRecommender(csv_path=csv_path)
    
    # モデルファイルパスを生成
    csv_basename = os.path.splitext(os.path.basename(csv_path))[0]
    model_filename = f'{csv_basename}_user_based_alpha_{alpha:.1f}.pkl'
    ...
    try:
        ...
        # モデルが存在しない場合は訓練
        with st.spinner(f"ユーザーベースモデルを訓練中です (α={alpha})..."):
...
def get_user_based_recommendation_reason(
    ...
    most_similar_user_id, similarity = similar_users[0]
    
    # 共通して聞いているアーティスト（上位3つ）を取得
    common_artists = recommender.get_common_artists(user_id, most_similar_user_id, top_n=3)
    
    return (most_similar_user_id, similarity), common_artists
...
def display_user_based_recommendations_with_reasons(
    ...
    if isinstance(recommendations, str):
        ...
        # 推薦理由を表示
        most_similar_user, common_artists = reason_info
        
        if most_similar_user:
            similar_user_id, similarity = most_similar_user
            st.info(f"🤝 **あなたと似ているユーザー(ID: {similar_user_id}, 類似度: {similarity:.3f})が聞いています**")
            
            if common_artists:
                st.markdown("**あなたと類似ユーザーが共通して聞いているアーティスト:**")
                for i, (artist, user_plays, similar_user_plays) in enumerate(common_artists, 1):
                    st.markdown(f"  {i}. **{artist}** (あなた: {user_plays}回, 類似ユーザー: {similar_user_plays}回)")
        else:
            st.info("💡 *一般的な人気に基づく推薦*")
...
def main():
    st.title("🎵 Music Recommender Demo (User-Based with Recommendation Reasons)")
    st.markdown("**ユーザーベース協調フィルタリング推薦システム - 推薦理由表示機能付き**")
    ...
    n_similar_users = st.sidebar.slider(
        "類似ユーザー数",
        min_value=5,
        max_value=50,
        value=10,
    ...
    # レコメンダーを読み込み
    try:
        recommender = load_user_based_recommender(csv_path, alpha)
    except Exception as e:
    ...
    with st.expander("データセット情報"):
        ...
        st.write("- **アルゴリズム**: ユーザーベース協調フィルタリング（ALS user_factors使用）")
        st.write(f"- **信頼度関数**: 1 + {alpha} × 再生回数")
        st.write("- **類似度計算**: コサイン類似度（ALS user_factors）")
        st.write("- **推薦理由**: 最も類似したユーザーの聴取履歴に基づく理由表示")
    ...
    if get_recommendations:
        ...
        # ユーザーベース推薦
        st.subheader("🎯 ユーザーベース推薦結果")
        
        with st.spinner("ユーザーベース推薦生成中..."):
    ...
    # フッター
    st.markdown("---")
    st.markdown("Built with Streamlit and User-Based Collaborative Filtering - With Recommendation Reasons")
